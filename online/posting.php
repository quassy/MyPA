...
if ($submit) {
  ...
  if ((("$subject"!="") || $reply || $edit)  
    ...
    if (!$reply && !$edit) {
	 ...
	 "subject='$subject',gal_id='$gal_id',creator='$myrow[leader]'";
	...
	$q = "INSERT INTO news set planet_id=1,date=now(),type=10,".
	   "text='Politics warning\npid: $Planetid\ntext=$text\n'";
	$result = mysql_query ($q, $db);
        do_log_me (3, 1, "Wrong politics post");
      }
    }
...
if ($reply) {
  ...
  echo "Politics: Reply to thread</th></tr>";
} else if ($edit) {
  ...
  echo "Politics: Edit Posting</th></tr>";
  
} else {
  echo "Politics: New thread</th></tr>";
}

?>

<tr><td class="f1" width="80">Subject:</th>
<td width="420">
<?php
if (!$subject || $subject == "") {
...
<?php
  if($mysettings & 2) {
    if ($reply)
      echo "<a href=\"politics.php?thread=$reply\">Return to thread</a>\n";
    else
      echo "<a href=\"politics.php\">Return to Politics</a>\n";
  } else {
