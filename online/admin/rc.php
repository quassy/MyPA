...
if (($research || $construction || $rc) && $name ) {
  ...
  if ($build_ticks) $values .= ",build_ticks='$build_ticks'";
  if ($construct_id) $values .= ",construct_id='$construct_id'";
  if ($research_id) $values .= ",research_id='$research_id'";
  if ($rc_id) $values .= ",rc_id='$rc_id'";
  if ($block_id) $values .= ",block_id='$block_id'";
...
$result = mysql_query("SELECT * FROM rc_class ORDER BY id",$db);
