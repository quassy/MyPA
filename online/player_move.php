...
function player_move($sx,$sy,$sz,$tx=0,$ty=0,$tz=0) {
  ...
  $res = mysqli_query("SELECT id FROM planet ".
...
function player_move_id ($id, $sx, $sy, $sz, $tx=0, $ty=0, $tz=0) {
  ...
  $res = mysqli_query("SELECT id FROM galaxy WHERE gc=$id AND ".
                     "x=$sx AND y=$sy", $db);
  ...
  mysqli_query("UPDATE planet set vote=0 WHERE vote='$id'", $db);
...
function insert_player ($id, $tx=0,$ty=0,$tz=0) {
  ...
  mysqli_query ("UPDATE planet SET x=$tx,y=$ty,z=$tz ".
               "WHERE id=$id", $db);
  mysqli_query ("UPDATE galaxy SET members=members+1 ".
